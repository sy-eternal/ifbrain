...
           "fnCreatedRow": function(nRow, aData, iDataIndex) {
	...
	return {
		...
		changepersonCount: function (id) {
			...
			if(personCount<=0){
				$("#check"+id).attr('disabled',true);//代表不可以点
			}else{
				$("#check"+id).attr('disabled',false);//代表可以点
			}
			//小计
		...
		changecheck: function(id){
			...
			if($("#check"+id).prop("checked")){
				...
				*/					
				...
				var check="check";
				$("input[name="+personCount+"]").attr('disabled',true);
				/*alert(check);*/
				...
				var personCount="personCount";
				var check="check";
				$("input[name="+personCount+"]").attr('disabled',false);
				$("input[name="+check+"]").attr('disabled',true);
				
				$("#personCount"+id).val(0);
				$("#subTotalAmount"+id).val(null);
